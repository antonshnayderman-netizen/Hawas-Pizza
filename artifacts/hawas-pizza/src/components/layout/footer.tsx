    ...
    <footer className="bg-foreground text-background py-20 px-6">
      <div className="container mx-auto max-w-7xl">
        <FadeIn className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 md:gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="flex items-center gap-3 mb-5">
              {/* Das Logo ist selbst fast schwarz – auf dem dunklen Footer
                  braucht es eine helle Kante, sonst verschwindet es. */}
              <img
                src={logoUrl}
                alt="Deine Pizza Logo"
                className="h-10 w-auto object-contain drop-shadow-[0_0_7px_rgba(255,255,255,0.45)]"
              />
              <h3 className="text-xl font-bold tracking-tight">Deine Pizza</h3>
            </div>
            <p className="text-background/60 text-sm leading-relaxed mb-6">
              Handwerkliche Steinofenpizza in Hamburg-Langenhorn. Nur zur Abholung oder Lieferung – kompromisslose Frische, puristischer Geschmack.
            </p>
            <Button
              size="md"
        ...
        <div className="mt-20 pt-8 border-t border-background/10 flex flex-col md:flex-row items-center justify-between gap-4 text-xs text-background/40">
          <p>© {new Date().getFullYear()} Deine Pizza. Alle Rechte vorbehalten.</p>
